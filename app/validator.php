...
if (!empty($_SESSION['activeAdmin']) && isset($_SESSION['schCode'])) {
    ...
    //Contact Data
    if (isset($_POST['Update_phone_form']) || isset($_POST['Update_email_form']) || isset($_POST['Update_address_form'])) {   
        ...
        if ($model->upDate($tblName, $contact_data, $condition) == true) {
            $user->recordLog($_SESSION['schCode'], 'Contact Data Validation', 'A validation remark has been added on the Contact information of the school with code : ' . $_SESSION['schCode']);
            $utility->notifier('success', 'You have Successfully submitted a validation remark for ' . $_SESSION['schCode']);
