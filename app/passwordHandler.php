...
// Check if password is a new variable
    if($utility->inputEncode($newpwd) != $user_details['user_password']){
    ...
    if($user_details['user_password'] === 'abcd1234'){
        if($oldpwd === $user_details['user_password'] && $newpwd == $confirmpwd && strlen($newpwd) >= 8 && strlen($newpwd) <=16 ){
            ...
            $password_data = [
                'user_password' => $utility->inputEncode($newpwd),
            ];
        ...
        if($utility->inputDecode($user_details['user_password']) === $oldpwd && $newpwd == $confirmpwd && strlen($newpwd) >= 8 && strlen($newpwd) <=16 ){
            $condition = [
                'user_name' => $_SESSION['active'],
            ];
            $password_data = [
